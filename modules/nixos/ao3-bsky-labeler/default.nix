...
in
{
  options.services.ao3-bsky-labeler = {
    enable = lib.mkEnableOption "A labeler for AO3 tags";
  ...
  config = mkIf cfg.enable {
    users = {
      ...
      groups = optionalAttrs (cfg.group == "ao3-bsky-labeler") { ao3-bsky-labeler = { }; };
    };
    ...
    # Trigger with `systemctl start ao3-bsky-labeler-reconcile`
    systemd.services.ao3-bsky-labeler-reconcile = {
      after = [
        "network.target"
        "ao3-bsky-labeler.service"
      ];
      ...
      script = ''
        ...
        export SIGNING_KEY="$(cat ${cfg.signingKeyFile})"
        exec ${cfg.package}/bin/ao3-bsky-labeler-reconcile
      '';
    };
